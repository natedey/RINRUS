...
def trim_pdb_models(sm,pdb_res_name,pdb_res_atom,Alist,ufree_atoms,mustadd,oldresatom):
    res_part_list = {}
    res_atom = {}
    res_info = {}
    ...
    # make sure everything from previous model added
    if oldresatom:
        for key in oldresatom.keys():
            if key in res_part_list.keys():
                for at in oldresatom[key]:
                    if at not in res_part_list[key]: res_part_list[key].append(at)
            else:
                res_part_list[key] = oldresatom[key]
    ...
    res_pick,res_info,res_atom = final_pick2(pdb,res_atom,res_info,sel_key)
...
if __name__ == '__main__':
    ...
    ### make max model first no matter what ###
    seedfroz, res_atom, res_info, res_pick = trim_pdb_models(lmax,pdb_res_name,pdb_res_atom,Alist,ufree_atoms,mustadd,None)
    write_model_files(lmax,res_atom,res_info,res_pick)

    ### print warning if any seed atoms frozen ###
    if seedfroz:
